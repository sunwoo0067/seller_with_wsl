...
class MockStorage(BaseStorage):
    ...
    async def list(
        self, table: str, filters: Optional[dict] = None, limit: int = 100
    ) -> List[dict]:
        """데이터 목록 조회"""
        if table not in self.data:
            return []
    ...
    def exists_by_hash(self, supplier_id: str, data_hash: str) -> bool:
        """해시로 중복 체크"""
        items = asyncio.run(
            self.list("raw_products", {"supplier_id": supplier_id, "data_hash": data_hash})
        )
        return len(items) > 0
    ...
    def get_stats(self, table: str, group_by: Optional[str] = None) -> Dict[str, Any]:
        ...
        stats = {"total": len(items), "by_status": {}}
    ...
    def upsert(self, table: str, data: Dict[str, Any], unique_fields: List[str]) -> str:
        """데이터 업서트"""
        # 기존 데이터 찾기
        items = asyncio.run(self.list(table))
