...
class TestOwnerclanIntegration:
    ...
    def test_full_pipeline(self, mock_storage, sample_auth_response, sample_list_response, sample_detail_response):
        """전체 파이프라인 테스트"""
        with patch('requests.post') as mock_post:
            # 인증 응답 설정
            auth_response = Mock()
            ...
            parsed_products = parser.parse_products({"data": {"allItems": {"edges": [{"node": p} for p in products]}}})
