...
class TestImageProcessor:
    ...
    def mock_router(self):
        ...
        router.select_model.return_value = Mock(
            model_name="gemini-flash", provider="gemini", supports_vision=True
        )
        return router
    ...
    def processor(self, mock_router):
        ...
        return ImageProcessor(
            model_router=mock_router, watermark_text="TEST", watermark_opacity=128
        )
    ...
    async def test_generate_caption(self, mock_execute_with_model, processor, test_image):
        ...
        mock_execute_with_model.return_value = {
            "content": "Mocked image caption",
            "usage": {"total_tokens": 50},
        }

        config = TaskConfig(task_type=TaskType.IMAGE_CAPTION, requires_vision=True)
    ...
    async def test_remove_background(self, mock_execute_with_model, processor, test_image):
        ...
        config = TaskConfig(task_type=TaskType.BACKGROUND_REMOVE, requires_vision=True)
    ...
    async def test_process_all(self, processor, test_image_path):
        """전체 처리 테스트"""
        result = await processor.process(test_image_path, process_type="all")
    ...
    async def test_process_watermark_only(self, processor, test_image):
        """워터마크만 처리 테스트"""
        result = await processor.process(test_image, process_type="watermark")
    ...
    async def test_process_with_options(self, processor, test_image):
        ...
        result = await processor.process(
            test_image, process_type="watermark", output_format="jpg", quality=80
        )
    ...
    async def test_process_batch_images(self, processor, tmp_path):
        ...
        results = await processor.process_batch_images(
            image_paths, process_type="watermark", max_concurrent=2
        )
