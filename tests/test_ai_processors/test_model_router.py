...
class TestModelRouter:
    ...
    def test_select_model_for_simple_task(self, router):
        """간단한 작업 모델 선택"""
        config = TaskConfig(
            task_type=TaskType.PRODUCT_NAME_ENHANCE, complexity="low", expected_tokens=100
        )
    ...
    def test_select_model_by_complexity(self, router):
        ...
        config_low = TaskConfig(
            task_type=TaskType.HTML_TO_TEXT, complexity="low", expected_tokens=200
        )
        ...
        config_high = TaskConfig(
            task_type=TaskType.DESCRIPTION_GENERATE, complexity="high", expected_tokens=1000
        )
    ...
    def test_budget_check(self, router):
        """예산 체크 테스트"""
        # 예산 내
        config = TaskConfig(task_type=TaskType.SEO_KEYWORDS, expected_tokens=1000)
        ...
        expensive_config = TaskConfig(
            task_type=TaskType.PRICE_ANALYSIS, expected_tokens=10000  # 예산 초과
        )
    ...
    def test_fallback_to_free_model(self, router):
        ...
        config = TaskConfig(task_type=TaskType.DESCRIPTION_GENERATE, expected_tokens=1000)
    ...
    def test_task_config_validation(self):
        ...
        # 정상 설정
        config = TaskConfig(
            task_type=TaskType.PRODUCT_NAME_ENHANCE, complexity="medium", expected_tokens=500
        )
        ...
        with pytest.raises(ValueError):
            TaskConfig(
                task_type=TaskType.PRODUCT_NAME_ENHANCE, complexity="very_high"  # 허용되지 않는 값
            )
