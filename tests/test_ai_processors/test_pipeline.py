...
from dropshipping.ai_processors.model_router import ModelRouter, TaskConfig
...
class TestAIProcessingPipeline:
    ...
    async def test_process_product_basic(self, pipeline, sample_product):
        ...
        with patch.object(pipeline.product_enhancer, 'process', new_callable=AsyncMock) as mock_enhance:
            ...
            # Mock ImageProcessor
            with patch.object(pipeline, '_process_product_images', new_callable=AsyncMock) as mock_images:
                mock_images.return_value = {
                    "image_0": {
                        "original_url": "https://example.com/image1.jpg",
                        "caption": "블루투스 이어폰 메인 이미지",
                        "processed_images": {
                            "watermarked": "processed/test-001_wm_0.png"
                        }
                    }
                }
    ...
    async def test_enhance_product_info_selective(self, pipeline, sample_product):
        ...
        with patch.object(pipeline.product_enhancer, 'process', new_callable=AsyncMock) as mock_enhance:
            ...
            result = await pipeline._enhance_product_info(
                sample_product,
                {"enhance_name": True, "generate_description": False}
            )
    ...
    async def test_process_product_with_error(self, pipeline, sample_product):
        ...
        with patch.object(pipeline, '_enhance_product_info', new_callable=AsyncMock) as mock_enhance:
    ...
    async def test_save_enhanced_product(self, pipeline, sample_product):
        ...
        processing_result = {
            ...
            "processed_images": {
                "image_0": {"watermarked": "path/to/image.png"}
            },
            "processing_time": 1.5,
    ...
    def test_log_pipeline_run(self, pipeline):
        ...
        with patch('dropshipping.ai_processors.pipeline.logger.error') as mock_error:
            with patch('dropshipping.ai_processors.pipeline.logger.info', side_effect=Exception("로그 오류")):
