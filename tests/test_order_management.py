...
class MockStorage:
    ...
    async def list(
        self, table: str, filters: Dict[str, Any] = None, limit: int = None
    ) -> List[Dict[str, Any]]:
...
class MockFetcher:
    ...
    async def fetch_product(self, product_id: str) -> Dict[str, Any]:
        return {"supplier_product_id": product_id, "stock": 50, "price": 10000}
...
class TestCoupangOrderManager:
    ...
    def manager(self, storage):
        config = {"api_key": "test_key", "api_secret": "test_secret", "seller_id": "test_seller"}
...
class TestDomemeOrderer:
    ...
    def test_build_order_xml(self, orderer):
        ...
        order = Order(
            ...
            delivery=DeliveryInfo(method="택배", status=DeliveryStatus.PENDING),
        )
...
class TestInventorySync:
    ...
    def sync(self, storage):
        config = {"safety_stock": 5, "batch_size": 10}
...
class TestDeliveryTracker:
    ...
    def tracker_manager(self, storage):
        config = {"carriers": ["cj", "hanjin", "lotte", "post"]}
...
class TestTrackerManager:
    ...
    @pytest.fixture
    def manager(self, storage):
        config = {"carriers": ["cj", "hanjin"], "batch_size": 10}
        return TrackerManager(storage, config)
