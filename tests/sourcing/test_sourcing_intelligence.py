...
async def setup_test_data(storage):
    ...
    for keyword in keywords:
        for i in range(30):
            ...
            keyword_daily_data.append({"keyword": keyword, "date": date, "volume": volume})
...
class TestSourcingDashboard:
    ...
    async def test_export_dashboard_data(self, setup_test_data):
        ...
        # JSON 내보내기
        json_data = await dashboard.export_dashboard_data(dashboard_type="overview", format="json")

        assert isinstance(json_data, str)
        assert len(json_data) > 0

        # CSV 내보내기
        csv_data = await dashboard.export_dashboard_data(dashboard_type="overview", format="csv")
...
class TestIntegration:
    ...
    async def test_opportunity_discovery_flow(self, setup_test_data):
        ...
        # 1. 키워드 기회 찾기
        keyword_researcher = KeywordResearcher(storage, {"env": "test"})
        keyword_opportunities = await keyword_researcher.find_opportunity_keywords("전자제품")
        ...
        # 4. 리포트 생성
        report = await dashboard.generate_report(report_type="weekly", categories=["전자제품"])
