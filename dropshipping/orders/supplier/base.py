...
class BaseSupplierOrderer(ABC):
    ...
    def __init__(
        self, supplier: SupplierType, storage: BaseStorage, config: Optional[Dict[str, Any]] = None
    ):
        """
        초기화
        ...
        # 통계
        self.stats = {"ordered": 0, "confirmed": 0, "failed": 0, "errors": []}
    ...
    @abstractmethod
    async def check_order_status(self, supplier_order_id: str) -> Dict[str, Any]:
        """
        공급사 주문 상태 확인
    ...
    @abstractmethod
    async def cancel_order(self, supplier_order_id: str, reason: str) -> bool:
        """
        공급사 주문 취소
    ...
    @abstractmethod
    async def get_tracking_info(self, supplier_order_id: str) -> Optional[Dict[str, Any]]:
        """
        배송 정보 조회
    ...
    async def process_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"공급사 주문 완료: {order_id} -> {supplier_order_id}")
                ...
                logger.error(f"공급사 주문 실패: {order_id} - {result.get('error')}")
                return False
    ...
    async def sync_order_status(self, order_id: str, supplier_order_id: str) -> bool:
        """
        공급사 주문 상태 동기화
        ...
        try:
            ...
            logger.info(f"공급사 주문 상태 업데이트: {order_id} - {status_info.get('status')}")
    ...
    async def process_batch_orders(self, order_ids: List[str]) -> Dict[str, bool]:
        """
        배치 주문 처리
        ...
        logger.info(f"배치 주문 완료: 성공 {success_count}/{len(order_ids)}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "total": total,
            "success_rate": (self.stats["ordered"] / total if total > 0 else 0),
        }
