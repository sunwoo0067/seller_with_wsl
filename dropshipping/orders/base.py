...
from dropshipping.models.order import Order, OrderStatus, OrderUpdate
...
class BaseOrderManager(ABC):
    ...
    async def process_new_orders(
        ...
        try:
            ...
            logger.info(
                f"주문 처리 완료: "
                f"조회 {len(raw_orders)}, 처리 {len(processed_orders)}"
            )
    ...
    async def sync_order_status(self, order_ids: Optional[List[str]] = None) -> int:
        ...
        for order_data in orders:
            try:
                # 최신 정보 조회
                raw_order = await self.fetch_order_detail(
                    order_data["marketplace_order_id"]
                )
                ...
                if latest_order.delivery.tracking_number != order_data["delivery"].get("tracking_number"):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "marketplace": self.marketplace.value,
            "success_rate": (
                self.stats["processed"] / self.stats["fetched"]
