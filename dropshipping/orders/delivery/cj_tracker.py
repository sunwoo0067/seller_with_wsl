...
class CJTracker(BaseDeliveryTracker):
    ...
    async def track(self, tracking_number: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.client.get(
                self.mobile_url, params={"paramInvcNo": tracking_number}
            )
    ...
    def _parse_tracking_html(self, html: str, tracking_number: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 현재 상태 추출
            status_match = re.search(r'class="state_ico.*?>(.*?)</span>', html, re.DOTALL)
            ...
            # 현재 위치 추출
            location_match = re.search(r"현재위치.*?<td>(.*?)</td>", html, re.DOTALL)
            ...
            # 배송 메시지
            message_match = re.search(r"배송상태.*?<td>(.*?)</td>", html, re.DOTALL)
    ...
    def _parse_tracking_history(self, html: str) -> List[Dict[str, Any]]:
        ...
        try:
            # 배송 이력 테이블 찾기
            table_match = re.search(r'<table.*?class="tbl_list".*?>(.*?)</table>', html, re.DOTALL)
