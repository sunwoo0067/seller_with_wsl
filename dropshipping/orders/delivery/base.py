...
class BaseDeliveryTracker(ABC):
    ...
    async def track_order(self, order_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if carrier != self.carrier.value:
                logger.debug(
                    f"다른 택배사입니다: {carrier} (현재: {self.carrier.value})"
                )
                return None
    ...
    async def _get_cached_tracking(
        ...
        if last_tracked:
            # 캐시 TTL 확인
            last_tracked_dt = datetime.fromisoformat(
                last_tracked.replace("Z", "+00:00")
            )
    ...
    async def _update_order_delivery(
        self,
        order_id: str,
        tracking_info: Dict[str, Any]
    ):
        ...
        if tracking_info.get("status") == DeliveryStatus.DELIVERED:
            updates["delivery.delivered_at"] = tracking_info.get(
                "delivered_at",
                datetime.now()
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "total": total,
            "success_rate": (
                self.stats["tracked"] / total if total > 0 else 0
            ),
            "delivery_rate": (
                self.stats["delivered"] / self.stats["tracked"]
                if self.stats["tracked"] > 0 else 0
            )
        }
