...
class MainScheduler:
    ...
    def _setup_jobs(self):
        """작업 설정"""
        # 일일 수집 (매일 오전 2시)
        self.add_job(DailyCollectionJob(None), CronTrigger(hour=2, minute=0), "daily_collection")

        # 주문 동기화 (30분마다)
        self.add_job(OrderSyncJob(None), IntervalTrigger(minutes=30), "order_sync")

        # 가격 조정 (매일 오전 3시)
        self.add_job(PriceAdjustmentJob(None), CronTrigger(hour=3, minute=0), "price_adjustment")

        # 재고 업데이트 (1시간마다)
        self.add_job(InventoryUpdateJob(None), IntervalTrigger(hours=1), "inventory_update")

        # 보고서 생성 (매일 오전 7시)
        self.add_job(ReportGenerationJob(None), CronTrigger(hour=7, minute=0), "report_generation")
    ...
    def add_job(self, job, trigger, job_id: str):
        ...
        self.scheduler.add_job(
            job_wrapper, trigger, id=job_id, replace_existing=True, max_instances=1
        )
    ...
    def run_job(self, job_id: str):
        ...
        if job_id in self.jobs:
            self.scheduler.add_job(
                self.jobs[job_id].run, id=f"{job_id}_manual", replace_existing=True
            )
