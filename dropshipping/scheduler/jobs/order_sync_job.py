...
class OrderSyncJob(BaseJob):
    ...
    def _init_order_managers(self) -> Dict[str, Any]:
        ...
        if "coupang" in self.marketplaces:
            managers["coupang"] = CoupangOrderManager(self.storage, self.config.get("coupang", {}))

        if "elevenst" in self.marketplaces:
            managers["elevenst"] = ElevenstOrderManager(
                self.storage, self.config.get("elevenst", {})
            )
    ...
    def _init_supplier_orderers(self) -> Dict[str, Any]:
        ...
        # 현재는 도메매만 지원
        orderers["domeme"] = DomemeOrderer(self.storage, self.config.get("domeme", {}))
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        for marketplace_name, manager in self.order_managers.items():
            try:
                orders = await self._collect_marketplace_orders(marketplace_name, manager)
    ...
    async def _collect_marketplace_orders(
        self, marketplace_name: str, manager: Any
    ) -> List[Dict[str, Any]]:
        """마켓플레이스 주문 수집"""
        ...
        try:
            if self.sync_mode == "incremental":
                ...
                # 전체 수집: 미처리 주문 전체
                orders = await manager.fetch_orders(status_filter=["paid", "preparing", "shipping"])
                ...
                for order in batch:
                    try:
                        ...
                        if existing_order:
                            ...
                            # 새 주문 저장
                            saved_order = await self.storage.create(
                                "orders",
                                {
                                    **order,
                                    "marketplace": marketplace_name,
    ...
    async def _forward_orders_to_suppliers(self, orders: List[Dict[str, Any]]):
        ...
        for order in orders:
            ...
            for item in order.get("items", []):
                ...
                supplier_orders[supplier].append({"order": order, "item": item, "product": product})
        ...
        for supplier_name, supplier_items in supplier_orders.items():
            ...
            supplier_stats = {"forwarded": 0, "failed": 0}
            ...
            for item_info in supplier_items:
                try:
                    ...
                    if len(batch_orders) >= 10:
                        await self._send_batch_orders(orderer, batch_orders, supplier_stats)
                        batch_orders = []
    ...
    async def _send_batch_orders(
        self, orderer: Any, batch_orders: List[Dict[str, Any]], stats: Dict[str, int]
    ):
        """배치 주문 전송"""
        try:
            # 공급사로 주문 전송
            ...
            for order_info in batch_orders:
                await self.storage.update(
                    "orders",
                    order_info["order_id"],
                    {"sync_status": "failed", "sync_error": str(e), "updated_at": datetime.now()},
                )
    ...
    async def _update_delivery_status(self):
        ...
        tracking_updates = await self.tracker_manager.track_bulk(
            [
                {"carrier": order.get("carrier", "cj"), "tracking_number": order["tracking_number"]}
    ...
    async def _notify_marketplace_delivery(self, order: Dict[str, Any]):
        ...
        if marketplace in self.order_managers:
            ...
            try:
                await manager.update_order_status(order["marketplace_order_id"], "delivered")
...
class OrderStatusCheckJob(BaseJob):
    ...
    async def _check_pending_orders(self, results: Dict[str, int]):
        ...
        pending_orders = await self.storage.list(
            "orders", filters={"sync_status": "pending", "created_at": {"$lte": threshold}}
        )
