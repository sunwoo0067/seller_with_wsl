        ...
        for product_id in product_ids:
            try:
                ...
                supplier_stock = await self._get_supplier_stock(
                    product["supplier"], product["supplier_product_id"]
                )
        ...
        try:
            ...
            if hasattr(fetcher, "get_stock"):
                ...
                if supplier not in self.stats["supplier_stats"]:
                    self.stats["supplier_stats"][supplier] = {"queried": 0, "failed": 0}
            ...
            logger.error(
                f"재고 조회 실패 - 공급사: {supplier}, 상품: {supplier_product_id}, 오류: {str(e)}"
            )
...
class RealTimeInventoryJob(BaseJob):
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        results = {"checked": 0, "updated": 0, "changes": []}
