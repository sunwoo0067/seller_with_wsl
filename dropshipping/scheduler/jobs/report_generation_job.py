...
class ReportGenerationJob(BaseJob):
    ...
    def __init__(self, storage: BaseStorage, config: Dict[str, Any] = None):
        ...
        self.include_sections = self.config.get(
            "sections", ["sales", "inventory", "orders", "pricing", "sourcing", "alerts"]
        )
        ...
        # 통계
        self.stats = {"sections_generated": 0, "formats_created": 0, "emails_sent": 0}
    ...
    async def validate(self) -> bool:
        ...
        existing_report = await self.storage.get(
            ...
            filters={
                ...
                "created_at": {"$gte": self._get_report_start_date(), "$lt": datetime.now()},
            },
        )
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        report_data = {
            "type": self.report_type,
            "period": {"start": report_period["start"], "end": report_period["end"]},
    ...
    async def _generate_sales_section(
        ...
        for order in orders:
            ...
            if marketplace not in marketplace_sales:
                marketplace_sales[marketplace] = {"revenue": Decimal("0"), "orders": 0}
                ...
                if product_id:
                    ...
                    if product:
                        ...
                        if category not in category_sales:
                            category_sales[category] = {"revenue": Decimal("0"), "items": 0}
        ...
        best_sellers = sorted(
            [
                {"product_id": pid, "quantity": data["quantity"], "revenue": float(data["revenue"])}
        ...
        return {
            ...
            "category_breakdown": {
                cat: {
                    ...
                    "items": data["items"],
                    "percentage": (
                        float(data["revenue"] / total_revenue * 100) if total_revenue > 0 else 0
                    ),
                }
        ...
        for product in active_products:
            ...
            if stock == 0:
                out_of_stock_products.append(
                    {"id": product["id"], "name": product["name"], "category": category}
                )
    ...
    async def _generate_orders_section(
        ...
        for order in orders:
            if order.get("forwarded_at") and order.get("order_date"):
                processing_time = (
                    order["forwarded_at"] - order["order_date"]
                ).total_seconds() / 3600
    ...
    async def _generate_pricing_section(
        ...
        price_adjustments = await self.storage.list(
            "price_adjustments", filters={"created_at": {"$gte": start_date, "$lte": end_date}}
        )
        ...
        for adjustment in price_adjustments:
            ...
            if product:
                ...
                if category not in category_adjustments:
                    category_adjustments[category] = {"count": 0, "total_change": Decimal("0")}
        ...
        return {
            ...
            "average_change_amount": (
                float(total_change_amount / total_adjustments) if total_adjustments > 0 else 0
            ),
            "category_breakdown": {
                cat: {
    ...
    async def _generate_alerts_section(
        ...
        alerts = await self.storage.list(
            "alerts", filters={"created_at": {"$gte": start_date, "$lte": end_date}}
        )
        ...
        # 중요 알림 추출
        critical_alerts = [
            {"type": alert["type"], "message": alert["message"], "created_at": alert["created_at"]}
            for alert in alerts
    ...
    def _generate_summary(self, sections: Dict[str, Any]) -> Dict[str, Any]:
        """리포트 요약 생성"""
        summary = {"highlights": [], "metrics": {}, "recommendations": []}
        ...
        if "sales" in sections:
            ...
            if sales["total_revenue"] > 0:
                summary["highlights"].append(f"총 매출 {sales['total_revenue']:,.0f}원 달성")
        ...
        if "inventory" in sections:
            ...
            if inventory["out_of_stock_count"] > 0:
                summary["highlights"].append(f"품절 상품 {inventory['out_of_stock_count']}개 발생")
                summary["recommendations"].append("품절 상품의 재입고 필요")
        ...
        if "orders" in sections:
            ...
            if sync_status.get("failed", 0) > 0:
                summary["highlights"].append(f"주문 동기화 실패 {sync_status['failed']}건")
                summary["recommendations"].append("실패한 주문 수동 확인 필요")
        ...
        if "pricing" in sections:
            ...
            if pricing["total_adjustments"] > 0:
                summary["highlights"].append(f"가격 조정 {pricing['total_adjustments']}건 실행")
        ...
        if "alerts" in sections:
            ...
            if critical_count > 0:
                summary["highlights"].append(f"긴급 알림 {critical_count}건 발생")
                summary["recommendations"].append("긴급 알림 즉시 확인 필요")
    ...
    async def _export_report(self, report_data: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        if "json" in self.report_formats:
            ...
            exported_files.append(
                {"format": "json", "filename": filename, "size": len(json_content)}
            )
        ...
        if "csv" in self.report_formats:
            ...
            exported_files.append({"format": "csv", "filename": filename, "size": len(csv_content)})
    ...
    async def _send_report_email(
        self, report_data: Dict[str, Any], exported_files: List[Dict[str, str]]
    ):
        """리포트 이메일 발송"""
        # TODO: 이메일 발송 구현
        # - SMTP 설정
...
class CustomReportJob(BaseJob):
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        # 리포트 저장
        saved_report = await self.storage.create(
            "reports",
            {
                "type": "custom",
