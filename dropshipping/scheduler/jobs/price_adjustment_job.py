...
class PriceAdjustmentJob(BaseJob):
    ...
    def __init__(
        ...
        self.adjustment_mode = self.config.get("adjustment_mode", "competitive")  # competitive/rule_based/ai
        self.min_margin_rate = self.config.get("min_margin_rate", 10.0)  # 최소 마진율 %
        self.max_price_change_rate = self.config.get("max_price_change_rate", 20.0)  # 최대 가격 변동률 %
        self.price_update_threshold = self.config.get("price_update_threshold", 100)  # 가격 변경 최소 단위
    ...
    async def _identify_target_products(self) -> List[Dict[str, Any]]:
        ...
        if self.adjustment_mode == "competitive":
            ...
            for product in active_products:
                ...
                if not last_adjustment or (
                    datetime.now() - last_adjustment["created_at"]
                ).days >= 7:
                    target_products.append(product)
            ...
            for marketplace_name, uploader in self.uploaders.items():
                try:
                    ...
                    if update_result["success"]:
                        # 가격 조정 기록
                        await self.storage.create("price_adjustments", {
                            "product_id": product["id"],
                            "marketplace": marketplace_name,
                            ...
                            "change_rate": (
                                ...
                                current_price_info["current_price"] * 100
                            ),
                            "adjustment_reason": current_price_info.get("adjustment_reason", "competitive"),
                            "status": "completed",
    ...
    async def _calculate_competitive_price(
        ...
        # 가격 포지셔닝 전략
        position_strategy = self.config.get("price_position", "competitive")  # competitive/premium/budget
    ...
    async def _analyze_price_changes(self):
        ...
        if today_adjustments:
            ...
            total_change_rate = sum(
                adj.get("change_rate", 0) for adj in today_adjustments
            )
