...
class DailyCollectionJob(BaseJob):
    ...
    def _init_fetchers(self) -> Dict[str, Any]:
        ...
        if "domeme" in self.suppliers:
            fetchers["domeme"] = DomemeFetcher(self.storage, self.config.get("domeme", {}))

        if "ownerclan" in self.suppliers:
            fetchers["ownerclan"] = OwnerclanFetcher(self.storage, self.config.get("ownerclan", {}))

        if "zentrade" in self.suppliers:
            fetchers["zentrade"] = ZentradeFetcher(self.storage, self.config.get("zentrade", {}))
        ...
        for supplier_name, fetcher in self.fetchers.items():
            try:
                await self._collect_from_supplier(
                    supplier_name, fetcher, self.transformers[supplier_name]
                )
            except Exception as e:
                logger.error(f"{supplier_name} 수집 실패: {str(e)}")
                self.stats["supplier_stats"][supplier_name] = {"status": "failed", "error": str(e)}
    ...
    async def _collect_from_supplier(self, supplier_name: str, fetcher: Any, transformer: Any):
        """공급사별 수집"""
        logger.info(f"{supplier_name} 수집 시작")

        supplier_stats = {"fetched": 0, "processed": 0, "failed": 0, "start_time": datetime.now()}

        try:
            # 최신 상품부터 수집
            products = await fetcher.fetch_products(
                limit=self.max_products_per_supplier, sort_by="newest"
            )
            ...
            for i in range(0, len(products), self.batch_size):
                ...
                for product in batch:
                    try:
                        ...
                        # 저장
                        await self.storage.save_processed_product(standard_product.dict())
                        ...
                        logger.error(f"{supplier_name} 상품 처리 실패: {str(e)}")
...
class IncrementalCollectionJob(BaseJob):
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        for supplier_name, fetcher in self.fetchers.items():
            try:
                ...
                if hasattr(fetcher, "fetch_updated_products"):
                    products = await fetcher.fetch_updated_products(since=since, limit=50)
        ...
        logger.info(
            f"증분 수집 완료: " f"수집 {stats['fetched']}개, " f"처리 {stats['processed']}개"
        )
