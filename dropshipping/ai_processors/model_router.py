...
class ModelRouter:
    ...
    def select_model(self, task_config: TaskConfig) -> Optional[ModelConfig]:
        ...
        if task_config.requires_vision:
            available_models = {
                name: config for name, config in self.models.items() if config.supports_vision
            }
        else:
    ...
    def _select_by_complexity(
        self, task_config: TaskConfig, available_models: Dict[str, ModelConfig]
        ...
        preferences = complexity_preferences.get(
            task_config.complexity, complexity_preferences["medium"]
        )
    ...
    def _fallback_to_free_model(
        ...
        if free_models:
            ...
            logger.warning(f"예산 초과로 무료 모델 사용: {model.model_name}")
            return model
    ...
    def get_usage_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_percentage": float(
                (self.current_usage / self.monthly_budget * 100) if self.monthly_budget > 0 else 0
            ),
        }
