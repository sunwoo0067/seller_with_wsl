...
class ProductEnhancer(BaseAIProcessor):
    ...
    async def process(
        ...
        **kwargs
        ...
        if enhance_type in ["all", "name"]:
            results["enhanced_name"] = await self._enhance_product_name(
                product, config
            )
        
        # 설명 생성
        if enhance_type in ["all", "description"]:
            results["generated_description"] = await self._generate_description(
                product, config
            )
        
        # SEO 키워드 추출
        if enhance_type in ["all", "seo"]:
            results["seo_keywords"] = await self._extract_seo_keywords(
                product, config
            )
    ...
    async def _enhance_product_name(
        self, 
