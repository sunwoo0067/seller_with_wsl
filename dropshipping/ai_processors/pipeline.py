...
class AIProcessingPipeline:
    ...
    async def _enhance_product_info(
        ...
        for enhance_type in enhance_types:
            try:
                result = await self.product_enhancer.process(
                    product,
    ...
    async def _process_product_images(
        ...
        images_to_process = [
            ...
            if img.url and (img.is_main or len(product.images) <= 5)
        ][:5]
        ...
        for idx, image in enumerate(images_to_process):
            try:
                ...
                result = {
                    ...
                    "processed_images": {
                        "watermarked": f"processed/{product.id}_wm_{idx}.png"
                    }
                }
    ...
    def _calculate_cost(self) -> Decimal:
        ...
        total_tokens = (
            enhancer_usage.get("total_tokens", 0) + 
            image_usage.get("total_tokens", 0)
        )
    ...
    async def process_batch(
        ...
        results = await asyncio.gather(
            *[process_with_semaphore(product) for product in products],
