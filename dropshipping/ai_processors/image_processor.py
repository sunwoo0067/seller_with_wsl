...
class ImageProcessor(BaseAIProcessor):
    ...
    async def process(
        ...
        **kwargs,
        ...
        if process_type in ["all", "background"]:
            ...
            if bg_removed:
                bg_path = self._save_image(bg_removed, "background_removed", output_format, quality)
        ...
        if process_type in ["all", "watermark"]:
            ...
            wm_path = self._save_image(watermarked, "watermarked", output_format, quality)
    ...
    def _load_image(
        self, data: Union[str, Path, Image.Image, Dict]
    ...
    async def _generate_caption(self, image: Image.Image, config: TaskConfig) -> str:
        """이미지 캡션 생성"""
...
4. 50-100자 내외로 간결하게 작성"""
        ...
        try:
            ...
            result = await self._execute_with_model(model, prompt, image=image_base64)
    ...
    async def _remove_background(
        self, image: Image.Image, config: TaskConfig
        ...
        try:
            ...
            result = await self._execute_with_model(
                model, image=image_base64, task="background_removal"
            )
    ...
    def _add_watermark(self, image: Image.Image) -> Image.Image:
        ...
        try:
            ...
            # 워터마크 그리기
            draw.text(
                (x, y), self.watermark_text, fill=(255, 255, 255, self.watermark_opacity), font=font
            )
    ...
    def _save_image(self, image: Image.Image, suffix: str, format: str, quality: int) -> Path:
        """이미지 저장"""
    ...
    async def process_batch_images(
        ...
        **kwargs,
        ...
        results = await self.process_batch(image_paths, task_config, max_concurrent, **kwargs)

        logger.info(
            f"배치 처리 완료: 성공 {len(results)}, " f"실패 {len(image_paths) - len(results)}"
        )
