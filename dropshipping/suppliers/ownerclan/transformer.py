...
class OwnerclanTransformer(BaseTransformer):
    ...
    def to_standard(self, raw_product: Dict[str, Any]) -> Optional[StandardProduct]:
        ...
        try:
            ...
            # 판매가 계산 (공급가 기준 30% 마진)
            selling_price = self._calculate_selling_price(original_price)

            # 이미지 처리
            images = self._process_images(raw_product.get("images", []))

            # 옵션 처리
            options, variants = self._process_options(
                raw_product["id"], raw_product.get("options", [])
            )
        ...
        for idx, url in enumerate(images):
            if url and isinstance(url, str):
                ...
                product_images.append(
                    ProductImage(
                        url=url, is_main=(idx == 0), order=idx  # 첫 번째 이미지를 메인으로
                    )
                )
    ...
    def _process_options(
        self, product_id: str, options_data: List[Dict[str, Any]]
    ) -> tuple[List[ProductOption], List[ProductVariant]]:
        """옵션 정보를 표준 형식으로 변환

        Args:
            product_id: 공급사 상품 ID
            options_data: 파싱된 옵션 목록
        ...
        for idx, opt in enumerate(options_data):
            ...
            # 1. 변형 상품(Variant) 생성
            variant_sku = f"OC-{product_id}-{idx + 1}"
            variant_price = opt.get("price")
            ...
            variants.append(
                ProductVariant(
                    ...
                    name=opt.get("name"),
                    options=attributes,
                    price=Decimal(str(variant_price)) if variant_price is not None else None,
                    stock=opt.get("quantity", 0),
                    barcode=None,  # 파서에서 바코드 정보 없음
                )
            )

            # 2. 상품 옵션(ProductOption) 그룹화
            for attr_name, attr_value in attributes.items():
                if attr_name not in option_groups:
                    option_groups[attr_name] = set()
                option_groups[attr_name].add(attr_value)

        # 3. ProductOption 객체 생성
        product_options = []
        for opt_name, opt_values in option_groups.items():
            product_options.append(
                ProductOption(name=opt_name, values=sorted(list(opt_values)), required=True)
            )
