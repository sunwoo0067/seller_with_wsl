...
from dropshipping.models.sourcing import (
    SourcingReport, MarketTrend, ProductOpportunity,
    SalesMetrics, TrendDirection
)
...
class SourcingDashboard:
    ...
    async def get_sales_dashboard(
        ...
        try:
            ...
            # 1. 기간별 판매 추이
            daily_sales = await self._get_daily_sales_trend(
                start_date,
                end_date
            )
            ...
            # 3. 상품별 성과
            product_performance = await self._get_product_performance(
                start_date,
                end_date
            )
            ...
            return {
                ...
                "patterns": {
                    ...
                    "weekday": await self._analyze_weekday_pattern(
                        start_date,
                        end_date
                    )
                },
    ...
    async def get_trend_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            # 1. 시장 트렌드 예측
            market_predictions = await self.trend_predictor.predict_trends(
                horizon_days=30
            )
            ...
            # 5. 리스크 분석
            risk_analysis = await self._analyze_trend_risks(
                market_predictions,
                emerging_trends
            )
    ...
    async def generate_report(
        ...
        try:
            ...
            # 데이터 수집
            sales_data = await self._collect_sales_data_for_report(
                start_date,
                end_date,
                categories
            )
            ...
            opportunities = await self._collect_opportunities_for_report(
                categories
            )
            
            competitors = await self._collect_competitor_data_for_report(
                categories
            )
            
            keywords = await self._collect_keyword_data_for_report(
                categories
            )
    ...
    async def _get_competition_summary(self) -> Dict[str, Any]:
        ...
        # 평균 포지션
        if price_positions:
            avg_position = sum(p.get("percentile", 50) for p in price_positions) / len(price_positions)
        else:
            avg_position = 50
        ...
        return {
            ...
            "competitiveness": "high" if avg_position < 30 else "medium" if avg_position < 70 else "low"
        }
    ...
    async def _analyze_customers(
        ...
        top_customers_revenue = sum(
            c["revenue"] for c in sorted_customers[:top_20_percent]
        )
        ...
        return {
            ...
            "average_order_value": total_revenue / len(orders) if orders else Decimal("0"),
            "pareto_ratio": float(top_customers_revenue / total_revenue * 100) if total_revenue > 0 else 0
        }
    ...
    async def _get_top_competitors(self) -> List[Dict[str, Any]]:
        ...
        # 경쟁 상품 수로 정렬
        top_competitors.sort(
            key=lambda x: x["competing_products"],
            reverse=True
        )
    ...
    async def _assess_price_war_risk(self) -> Dict[str, Any]:
        ...
        price_drops = sum(1 for c in recent_changes if c.get("price", 0) < c.get("previous_price", 0))
    ...
    async def _analyze_keyword_performance(self) -> Dict[str, Any]:
        ...
        # 상위 상품의 키워드 분석
        top_products = await self.storage.list(
            "products",
            filters={"status": "active"},
            limit=50
        )
        ...
        top_performers = sorted(
            [
                {"keyword": k, **v}
                for k, v in keyword_performance.items()
            ],
    ...
    async def _analyze_keyword_combinations(self) -> List[Dict[str, Any]]:
        ...
        combinations = await self.keyword_researcher.analyze_keyword_combinations(
            base_keywords
        )
    ...
    async def _generate_keyword_insights(
        ...
        if opportunities:
            ...
            if low_comp_opps:
                insights.append(
                    f"{len(low_comp_opps)}개의 저경쟁 고검색량 키워드를 발견했습니다"
                )
        ...
        if total_potential > 10000000:
            insights.append(
                f"상위 10개 기회 키워드의 월 수익 잠재력: "
                f"{total_potential:,.0f}원"
            )
        ...
        for category in categories:
            ...
            if trends:
                ...
                forecasts.append({
                    ...
                    "confidence": trend.confidence_level,
                    "opportunities": len([
                        p for p in trend.trending_products
                        if p.get("score", 0) > 70
                    ])
                })
    ...
    async def _analyze_trend_risks(
        self,
        market_predictions: List[MarketTrend],
    ...
    async def _extract_trend_opportunities(
        self,
        trend: MarketTrend
    ) -> List[Dict[str, Any]]:
        """트렌드별 기회 추출"""
        ...
        for keyword in trend.trending_keywords[:5]:
            metrics = await self.keyword_researcher.research_keyword(
                keyword,
                trend.category
            )
    ...
    def _extract_key_findings(
        ...
        if metrics and metrics.growth_rate > 30:
            findings.append(
                f"매출이 {metrics.growth_rate:.1f}% 급성장했습니다"
            )
        ...
        if competitors:
            ...
            if avg_reviews > 1000:
                findings.append(
                    "주요 경쟁사들의 고객 리뷰가 매우 활발합니다"
                )
        
        # 키워드 관련
        low_comp_keywords = [
            k for k in keywords
            if k.get("metrics", {}).get("competition_level") == "low"
        ]
        if len(low_comp_keywords) > 5:
            findings.append(
                f"{len(low_comp_keywords)}개의 저경쟁 키워드 기회가 있습니다"
            )
    ...
    def _generate_recommendations(
        ...
        if metrics and metrics.trend == TrendDirection.UP:
            recommendations.append(
                "현재의 성장 모멘텀을 활용하여 인기 카테고리 상품을 확대하세요"
            )
        elif metrics and metrics.trend == TrendDirection.DOWN:
            recommendations.append(
                "매출 하락 원인을 분석하고 프로모션 전략을 재검토하세요"
            )
        
        # 트렌드 활용
        if trends:
            up_trend_categories = [
                t.category for t in trends
        ...
        if opportunities:
            low_barrier_opps = [
                ...
                if o.entry_barrier == "low" and o.opportunity_score > 70
            ]
            
            if low_barrier_opps:
                recommendations.append(
                    f"진입 장벽이 낮은 {len(low_barrier_opps)}개 기회를 "
                    "우선적으로 검토하세요"
                )
