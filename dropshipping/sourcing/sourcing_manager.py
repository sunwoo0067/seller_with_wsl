...
from dropshipping.models.sourcing import (
    ProductOpportunity, SourcingReport, MarketTrend, SalesMetrics, TrendDirection
)
...
class SourcingManager:
    ...
    def __init__(
        ...
        # 설정
        self.opportunity_threshold = self.config.get("opportunity_threshold", 70)  # 기회 점수 임계값
    ...
    async def find_product_opportunities(
        ...
        try:
            ...
            for category in categories:
                tasks.append(self._find_opportunities_for_category(category, trends_by_category.get(category), min_score))
    ...
    async def _find_opportunities_for_category(
        self,
        category: str,
        category_trend: Optional[MarketTrend],
        min_score: float
    ) -> List[ProductOpportunity]:
        ...
        for keyword_data in opportunity_keywords[:10]:  # 상위 10개
            ...
            competition_analysis, profitability, supplier_analysis = await asyncio.gather(
                ...
                self._analyze_profitability(keyword, metrics, await self._analyze_keyword_competition(keyword, category)),
    ...
    async def generate_sourcing_report(
        ...
        try:
            ...
            # 1. 판매 분석
            sales_metrics = await self._generate_sales_summary(
                categories,
                start_date,
                end_date
            )
            ...
            # 2. 시장 트렌드
            all_market_trends = await self.sales_analyzer.detect_trends(
                lookback_days=days
            )
            ...
            opportunities = await self.find_product_opportunities(
                categories,
                min_score=self.opportunity_threshold
            )
            
            # 4. 경쟁사 동향
            competitor_updates = await self._get_competitor_updates(
                categories,
                start_date
            )
            
            # 5. 키워드 인사이트
            keyword_insights = await self._generate_keyword_insights(
                categories,
                days
            )
    ...
    async def _get_opportunities_to_monitor(self, opportunity_ids: Optional[List[str]]) -> List[ProductOpportunity]:
        """모니터링할 기회 목록을 가져옵니다."""
        if opportunity_ids:
            opportunities = []
    ...
    async def _process_single_opportunity_monitoring(self, opportunity: ProductOpportunity) -> Dict[str, Any]:
    ...
    def _generate_monitoring_summary(self, opportunities: List[ProductOpportunity], monitoring_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """모니터링 결과를 요약합니다."""
        return {
            "total_monitored": len(opportunities),
            "action_required": sum(1 for r in monitoring_results if r["action_required"]),
            "expired": sum(1 for o in opportunities if o.expires_at and o.expires_at < datetime.now()),
            "high_potential": sum(1 for r in monitoring_results if r["current_status"]["score"] >= 85)
        }
    ...
    async def _analyze_keyword_competition(
        self,
        keyword: str,
        category: str
    ) -> Dict[str, Any]:
    ...
    async def _analyze_profitability(
        ...
        estimated_cost, estimated_margin = self._calculate_estimated_costs_and_margins(recommended_price)
        estimated_sales, monthly_revenue, monthly_profit = self._calculate_estimated_sales_and_profit(metrics, competition, recommended_price, estimated_margin)
    ...
    def _calculate_estimated_costs_and_margins(self, recommended_price: Decimal) -> Tuple[Decimal, Decimal]:
        """예상 원가 및 마진을 계산합니다."""
    ...
    def _calculate_estimated_sales_and_profit(self, metrics: Any, competition: Dict[str, Any], recommended_price: Decimal, estimated_margin: Decimal) -> Tuple[int, Decimal, Decimal]:
        """예상 판매량 및 월별 수익을 계산합니다."""
        market_share = 0.1 if competition["market_saturation"] == "low" else 0.05
        estimated_sales = int(
            metrics.search_volume * 
            (metrics.conversion_rate or 0.02) * 
            market_share
        )
    ...
    def _filter_recommended_suppliers(self, suppliers: List[Dict[str, Any]], keyword: str) -> List[str]:
    ...
    def _calculate_profitability_score(self, profitability: Dict[str, Any]) -> float:
        """수익성 점수를 계산합니다."""
        margin_rate = float(
            profitability["estimated_margin"] / 
            profitability["recommended_price"]
        ) if profitability["recommended_price"] > 0 else 0
    ...
    async def _generate_sales_summary(
        ...
        total_sales, total_revenue, average_price = self._calculate_overall_sales_metrics(all_orders)
    ...
    async def _get_all_orders_for_summary(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
    ...
    def _calculate_overall_sales_metrics(self, orders: List[Dict[str, Any]]) -> Tuple[int, Decimal, Decimal]:
    ...
    async def _get_top_keywords_for_category(self, category: str, days: int) -> List[Dict[str, Any]]:
    ...
    async def _analyze_keyword_trends(self, top_keywords: List[Dict[str, Any]], days: int) -> Dict[str, Any]:
        """키워드 트렌드를 분석합니다."""
    ...
    def _get_sales_findings(self, sales_metrics: SalesMetrics) -> List[str]:
        ...
        if sales_metrics and sales_metrics.total_sales > 0:
            return [f"총 {sales_metrics.total_sales}개 판매, 매출 {sales_metrics.total_revenue:,.0f}원 달성"]
        return []
    ...
    def _get_opportunity_recommendations(self, opportunities: List[ProductOpportunity]) -> List[str]:
        """기회 기반 추천을 생성합니다."""
        if opportunities:
            top_opp = opportunities[0]
            return [f"{top_opp.category} 카테고리의 '{top_opp.keywords[0]}' 상품 출시 검토 (예상 월 수익: {top_opp.estimated_margin * top_opp.estimated_monthly_sales:,.0f}원)"]
        return []
    ...
    def _get_trend_recommendations(self, trends: List[MarketTrend]) -> List[str]:
        ...
        if up_trends:
            return [f"{up_trends[0].category} 카테고리 확대 고려 (상승 트렌드 {up_trends[0].strength:.0f}% 강도)"]
        return []

    def _get_competition_avoidance_recommendations(self, opportunities: List[ProductOpportunity]) -> List[str]:
        """경쟁 회피 추천을 생성합니다."""
    ...
    def _get_supplier_diversification_recommendations(self, opportunities: List[ProductOpportunity]) -> List[str]:
        """공급사 다각화 추천을 생성합니다."""
    ...
    async def _save_metrics(self, metrics: Dict[str, Any]) -> str:
        ...
        await self.storage.save("report_metrics", {
    ...
    async def _reevaluate_opportunity(
        self,
        opportunity: ProductOpportunity
    ) -> Dict[str, Any]:
        """기회 재평가"""
        current_metrics, current_competition = await self._get_current_opportunity_metrics(opportunity)
    ...
    async def _get_current_opportunity_metrics(self, opportunity: ProductOpportunity) -> Tuple[Any, Dict[str, Any]]:
        """현재 키워드 지표와 경쟁 상황을 가져옵니다."""
        current_metrics = await self.keyword_researcher.research_keyword(
            opportunity.keywords[0],
    ...
    def _calculate_score_change(self, opportunity: ProductOpportunity, current_metrics: Any, current_competition: Dict[str, Any]) -> float:
        """점수 변화를 계산합니다."""
        score_change = 0
    ...
    def _calculate_score_difference(self, opportunity: ProductOpportunity, current_status: Dict[str, Any]) -> float:
        """기회 점수 차이를 계산합니다."""
        return current_status["score"] - opportunity.opportunity_score

    def _check_competition_change(self, opportunity: ProductOpportunity, current_status: Dict[str, Any]) -> bool:
        """경쟁 수준 변화를 확인합니다."""
        return current_status["competition"] != opportunity.competition_level

    def _is_significant_change(self, score_change: float, opportunity: ProductOpportunity, current_status: Dict[str, Any]) -> bool:
        """중요한 변화인지 판단합니다."""
        if abs(score_change) > 15:
            return True
        if (opportunity.competition_level == "low" and 
            current_status["competition"] == "high"):
            return True
        return False
