...
class CompetitorMonitor:
    ...
    async def _get_product_info_for_competitor_identification(self, product_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _search_competitors_across_marketplaces(
        ...
        if marketplace:
            ...
            for mp in ["coupang", "11st", "gmarket", "smartstore"]:
                tasks.append(self._find_competitors_by_marketplace(
                    mp, category, keywords, price_range
                ))
    ...
    def _sort_and_limit_competitors(self, competitors: List[CompetitorInfo]) -> List[CompetitorInfo]:
        """경쟁사를 정렬하고 제한합니다."""
        return sorted(
            competitors,
    ...
    async def monitor_competitor_prices(
        ...
        try:
            ...
            price_changes = await asyncio.gather(*[self._detect_price_change(c) for c in competitors])
    ...
    def _calculate_our_product_position(self, our_price: Decimal, competitor_prices: List[Decimal]) -> Dict[str, Any]:
        ...
        return {"rank": position, "percentile": percentile, "competitiveness": price_competitiveness}
    ...
    def _build_shipping_strategy(self, competitor: CompetitorInfo, portfolio: Dict[str, Any]) -> Dict[str, Any]:
        """배송 전략을 구성합니다."""
        return {
    ...
    async def track_new_competitors(
        ...
        try:
            ...
            new_competitors_analysis = await self._identify_and_analyze_new_competitors(existing_ids, current_competitors)
    ...
    async def _get_existing_competitor_ids(self, category: str, marketplace: Optional[str]) -> Set[str]:
        """기존 경쟁사 ID 목록을 가져옵니다."""
    ...
    async def _identify_and_analyze_new_competitors(self, existing_ids: Set[str], current_competitors: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """신규 경쟁사를 식별하고 분석합니다."""
    ...
    async def _find_competitors_by_marketplace(
        ...
        similar_products = await self.storage.list(
            ...
            filters={
                ...
                "category": category,
                "price": {
                    "$gte": float(price_range[0]),
                    "$lte": float(price_range[1])
                }
            },
            limit=50
        )
    ...
    async def _detect_price_change(
        ...
        history = await self.storage.list(
            ...
            filters={"competitor_id": competitor.competitor_id},
            limit=1
        )
        
        if not history:
            # 첫 기록
            await self.storage.save("competitor_price_history", {
    ...
    async def _get_competitor_info(
        self,
        competitor_id: str
    ...
    async def _analyze_product_portfolio(
        self,
        competitor_id: str
    ) -> Dict[str, Any]:
        """상품 포트폴리오 분석"""
        ...
        products = await self.storage.list(
            "competitor_products",
            filters={"seller_id": competitor_id}
        )
    ...
    async def _analyze_promotion_strategy(
        self,
        competitor_id: str
    ) -> Dict[str, Any]:
        """프로모션 전략 분석"""
        ...
        avg_discount = (
            total_discount / len(promotions) if promotions else Decimal("0")
        )
    ...
    def _estimate_free_shipping_threshold(
        self,
        portfolio: Dict[str, Any]
    ) -> Optional[int]:
        """무료배송 기준 추정"""
    ...
    def _analyze_swot(
        ...
        # 강점
        if customer_satisfaction["average_rating"] and customer_satisfaction["average_rating"] >= 4.5:
        ...
        if pricing_strategy["strategy"] == "premium" and customer_satisfaction["average_rating"] and customer_satisfaction["average_rating"] < 4.0:
    ...
    async def _analyze_new_competitor(
        self,
