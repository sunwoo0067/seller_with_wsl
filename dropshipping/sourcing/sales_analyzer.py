...
from dropshipping.models.sourcing import MarketTrend, SalesMetrics, TrendDirection
...
class SalesAnalyzer:
    ...
    async def analyze_product_sales(
        self, product_id: str, start_date: datetime, end_date: datetime
    ) -> SalesMetrics:
        """
        상품별 판매 분석
        ...
        """
        try:
            orders, total_sales, total_revenue, average_price = (
                await self._get_product_orders_and_calculate_metrics(
    ...
    async def analyze_category_sales(
        ...
        try:
            ...
            product_metrics = await self._get_category_products_and_metrics(
                products, start_date, end_date
            )

            total_sales, total_revenue, top_products = self._calculate_category_overall_metrics(
                product_metrics
            )
    ...
    async def _get_category_products_and_metrics(
        self, products: List[Dict[str, Any]], start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
        ...
        for product in products:
            ...
            product_metrics.append(
                {"product_id": product["id"], "product_name": product["name"], "metrics": metrics}
            )
        return product_metrics

    def _calculate_category_overall_metrics(
        self, product_metrics: List[Dict[str, Any]]
        ...
        top_products = sorted(
            product_metrics, key=lambda x: x["metrics"].total_sales, reverse=True
        )[:10]
    ...
    def _calculate_price_distribution(
        ...
        """가격 분포를 계산합니다."""
        prices = [
            m["metrics"].average_price for m in product_metrics if m["metrics"].average_price > 0
        ]
        if not prices:
            return {
                "min": Decimal("0"),
    ...
    async def detect_trends(self, lookback_days: int = 30) -> List[MarketTrend]:
        """
        시장 트렌드 감지
        ...
        try:
            ...
            overall_trend = await self._analyze_overall_market_trend(start_date, end_date)
            if overall_trend:
    ...
    async def calculate_seasonality(self, product_id: str) -> Dict[str, float]:
        """
        계절성 분석
    ...
    def _calculate_seasonality_index(self, monthly_sales: Dict[int, List[int]]) -> Dict[str, float]:
        ...
        seasonality = {}
        total_avg = (
            sum(sum(sales) for sales in monthly_sales.values()) / 12 if monthly_sales else 100
        )
    ...
    async def _get_product_orders(
        self, product_id: Optional[str], start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
        """상품 주문 조회"""
    ...
    async def _get_all_orders_in_period(
    ...
    async def _get_previous_period_sales(
        self, product_id: Optional[str], current_start_date: datetime, current_end_date: datetime
    ) -> int:
        """이전 기간의 판매량을 가져옵니다."""
        ...
        if product_id:
            ...
            return sum(
                item.get("quantity", 0) for order in prev_orders for item in order.get("items", [])
            )
    ...
    async def _estimate_market_share(
        self, product_id: str, start_date: datetime, end_date: datetime, product_sales: int
    ) -> float:
        """시장 점유율 추정 (간단 구현)"""
    ...
    async def _get_category_total_sales(
        self, category: str, start_date: datetime, end_date: datetime
    ) -> int:
    ...
    def _empty_metrics(self, start_date: datetime, end_date: datetime) -> SalesMetrics:
        """빈 판매 지표"""
        return SalesMetrics(
            total_sales=0,
    ...
    async def _get_active_categories(self) -> List[str]:
        ...
        categories = {
            product.get("category_name") for product in products if product.get("category_name")
        }
    ...
    def _analyze_trend(self, daily_sales: List[Tuple[datetime, int]]) -> Dict[str, Any]:
        """트렌드 분석"""
    ...
    async def _get_product_sales_metrics(
        self, products: List[Dict[str, Any]], start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
    ...
    async def _analyze_overall_market_trend(
        ...
        try:
            ...
            while current <= end_date:
                ...
                day_sales = sum(
                    item.get("quantity", 0) for order in orders for item in order.get("items", [])
                )
                daily_sales.append((current, day_sales))
                current = next_day
            ...
            for category in categories[:5]:
                keywords = await self._extract_trending_keywords(category, start_date, end_date)
