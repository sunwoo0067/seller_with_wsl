...
from dropshipping.models.sourcing import MarketTrend, ProductOpportunity, TrendDirection
...
class TrendPredictor:
    ...
    async def predict_trends(
        self, category: Optional[str] = None, horizon_days: int = 30
    ) -> List[MarketTrend]:
        """
        트렌드 예측
        ...
        try:
            ...
            for cat in categories:
                ...
                seasonality_signal = self._analyze_seasonality_signal(
                    seasonality_data, datetime.now()
                )
                ...
                if prediction["confidence"] >= self.min_confidence:
                    ...
                    opportunities = await self._identify_opportunities(
                        cat, prediction, sales_data, keyword_data
                    )

                    # 5. 트렌드 생성
                    trend = self._create_trend(cat, prediction, opportunities, horizon_days)
            ...
            # 7. 우선순위 정렬
            all_trends.sort(key=lambda x: x.strength * x.confidence_level, reverse=True)
    ...
    async def predict_product_performance(
        ...
        try:
            ...
            # 과거 판매 데이터
            sales_history = await self._get_product_sales_history(product_id, days=90)
            ...
            forecast = self._forecast_sales(
                sales_history, trend_analysis, seasonality, keyword_trend, competition, horizon_days
            )

            # 리스크 분석
            risks = self._analyze_risks(product, trend_analysis, competition, forecast)

            # 추천사항
            recommendations = self._generate_recommendations(product, forecast, risks)
    ...
    async def identify_emerging_trends(self, min_growth_rate: float = 50.0) -> List[Dict[str, Any]]:
        """
        신흥 트렌드 식별
        ...
        while current <= end_date:
            ...
            orders = await self.storage.list(
                ...
                filters={
                    ...
                    "order_date": {"$gte": current, "$lt": next_day},
                },
            )
            ...
            for order in orders:
                for item in order.get("items", []):
                    product = await self.storage.get("products", item.get("product_id"))
    ...
    async def _collect_keyword_data(self, category: str) -> Dict[str, Any]:
        """키워드 데이터 수집"""
        # 카테고리 관련 키워드
        keywords = await self.keyword_researcher.find_opportunity_keywords(
            category, min_volume=100, max_competition="high"
        )
        ...
        for kw_data in keywords[:20]:  # 상위 20개
            ...
            # 30일 트렌드
            trend_data = await self.keyword_researcher.track_keyword_trends([keyword], days=30)
    ...
    async def _collect_competitor_data(self, category: str) -> List[Dict[str, Any]]:
        """경쟁사 데이터 수집"""
        # 카테고리 상품 샘플
        products = await self.storage.list(
            "products", filters={"category_name": category}, limit=10
        )
        ...
        for product in products:
            ...
            for comp in competitors[:5]:  # 상위 5개
                ...
                competitor_data.append({"competitor": comp, "price_data": price_monitor})

        return competitor_data

    async def _analyze_seasonality(self, category: str) -> Dict[str, float]:
        """계절성 분석"""
        # 카테고리 대표 상품들의 계절성
        products = await self.storage.list(
        ...
        for product in products:
            seasonality = await self.sales_analyzer.calculate_seasonality(product["id"])
    ...
    async def _collect_external_signals(self, category: str) -> Dict[str, Any]:
    ...
    def _analyze_sales_signal(
        self, sales_data: List[Tuple[datetime, int, Decimal]]
    ) -> Dict[str, Any]:
        """판매 신호 분석"""
        if len(sales_data) < 7:
            return {"strength": 0.0, "direction": TrendDirection.STABLE, "confidence": 0.0}
    ...
    def _analyze_keyword_signal(self, keyword_data: Dict[str, Any]) -> Dict[str, Any]:
        """키워드 신호 분석"""
        if not keyword_data:
            return {"strength": 0.0, "direction": TrendDirection.STABLE, "confidence": 0.0}
    ...
    def _analyze_competitor_signal(self, competitor_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """경쟁사 신호 분석"""
        ...
        if not price_changes:
            return {"strength": 0.0, "direction": TrendDirection.STABLE, "confidence": 50.0}
    ...
    def _analyze_seasonality_signal(
        self, seasonality_data: Dict[str, float], current_date: datetime
    ) -> Dict[str, Any]:
        """계절성 신호 분석"""
    ...
    def _analyze_external_signal(self, external_signals: Dict[str, Any]) -> Dict[str, Any]:
        """외부 신호 분석"""
        # SNS 언급량
        ...
        return {"strength": strength, "direction": direction, "confidence": confidence}
    ...
    async def _identify_opportunities(
        ...
        for keyword, data in keyword_data.items():
            ...
            if (
                metrics.competition_level in ["low", "medium"]
                and metrics.trend == TrendDirection.UP
            ):

                opportunity = ProductOpportunity(
                    opportunity_id=f"OPP_{category}_{keyword}_{datetime.now().strftime('%Y%m%d')}",
                    opportunity_score=self._calculate_opportunity_score(
                        metrics, prediction["strength"]
                    ),
    ...
    async def _analyze_cross_category_trends(
        self, category_trends: List[MarketTrend]
    ) -> List[MarketTrend]:
        """교차 카테고리 트렌드 분석"""
        cross_trends = []
        ...
        cross_keywords = {k: v for k, v in keyword_categories.items() if len(v) >= 2}
    ...
    def _analyze_time_series(self, sales_history: List[Tuple[datetime, int]]) -> Dict[str, Any]:
        """시계열 분석"""
        if len(sales_history) < 2:
            return {"trend": TrendDirection.STABLE, "average": 0, "volatility": 0, "slope": 0}
        ...
        return {"trend": trend, "average": average, "volatility": volatility, "slope": slope}

    async def _analyze_product_seasonality(self, product_id: str) -> Dict[str, float]:
        """상품 계절성 분석"""
        return await self.sales_analyzer.calculate_seasonality(product_id)

    async def _analyze_product_keywords(self, product: Dict[str, Any]) -> Dict[str, Any]:
        """상품 키워드 트렌드 분석"""
        # 상품명에서 주요 키워드 추출
        ...
        # 키워드 트렌드 조회
        trends = await self.keyword_researcher.track_keyword_trends(keywords, days=30)
        ...
        down_count = sum(
            1 for k, v in keyword_trends.items() if v.get("trend") == TrendDirection.DOWN
        )
    ...
    async def _analyze_product_competition(self, product_id: str) -> Dict[str, Any]:
        """상품 경쟁 분석"""
    ...
    def _forecast_sales(
        ...
        """판매량 예측"""
        if len(sales_history) < 30:
            return {"error": "예측을 위한 충분한 데이터가 없습니다", "confidence": 0}
        ...
        # 최종 예측
        forecast_daily = (
            recent_avg * trend_factor * seasonal_factor * keyword_factor * competition_factor
        )
    ...
    def _generate_recommendations(
        self, product: Dict[str, Any], forecast: Dict[str, Any], risks: List[Dict[str, Any]]
    ) -> List[str]:
        """추천사항 생성"""
        recommendations = []
    ...
    async def _detect_rising_keywords(self, min_growth_rate: float) -> List[Dict[str, Any]]:
        """급상승 키워드 감지"""
        rising_keywords = []

        # 모든 활성 키워드 조회
        keyword_metrics = await self.storage.list(
            "keyword_metrics", filters={"analyzed_at": {"$gte": datetime.now() - timedelta(days=7)}}
        )
    ...
    async def _detect_growing_categories(self, min_growth_rate: float) -> List[Dict[str, Any]]:
        """성장 카테고리 감지"""
        growing_categories = []
        ...
        for category in categories:
            ...
            previous_metrics = await self.sales_analyzer.analyze_category_sales(
                category, datetime.now() - timedelta(days=60), datetime.now() - timedelta(days=30)
            )
    ...
    async def _detect_rising_sellers(self, min_growth_rate: float) -> List[Dict[str, Any]]:
        """급부상 셀러 감지"""
        # 실제로는 셀러별 판매 추적 필요
        # 여기서는 빈 리스트 반환
        return []

    async def _match_trend_pattern(self, trend_data: Dict[str, Any]) -> Optional[str]:
        """트렌드 패턴 매칭"""
    ...
    async def _find_keyword_opportunities(self, keyword: str) -> List[str]:
        """키워드 관련 기회 찾기"""
        opportunities = []
    ...
    async def _find_category_opportunities(self, category: str) -> List[str]:
        """카테고리 관련 기회 찾기"""
        opportunities = []
    ...
    def _assess_trend_risk(self, trend_data: Dict[str, Any]) -> str:
        """트렌드 리스크 평가"""
        growth_rate = trend_data.get("growth_rate", 0)
    ...
    def _generate_trend_recommendation(self, trend_type: str, trend_data: Dict[str, Any]) -> str:
        """트렌드 추천사항 생성"""
    ...
    def _calculate_opportunity_score(self, metrics: Any, trend_strength: float) -> float:
        """기회 점수 계산"""
        score = 0.0
    ...
    def _identify_opportunity_risks(self, metrics: Any, prediction: Dict[str, Any]) -> List[str]:
        """기회 리스크 식별"""
        risks = []
    ...
    async def _get_social_mentions(self, category: str) -> Dict[str, Any]:
        """SNS 언급량 조회 (시뮬레이션)"""
        # 실제로는 SNS API 연동
        return {"count": 1000, "trend": 20, "sentiment": 0.7}  # 20% 증가  # 긍정적

    async def _get_news_sentiment(self, category: str) -> Dict[str, Any]:
        """뉴스 감성 분석 (시뮬레이션)"""
        # 실제로는 뉴스 API 연동
        return {"score": 15, "article_count": 50}  # -100 ~ 100

    async def _get_search_trend(self, category: str) -> Dict[str, Any]:
        """검색 트렌드 조회 (시뮬레이션)"""
        # 실제로는 검색 트렌드 API
        return {"growth": 25, "volume_index": 80}  # 25% 성장  # 0-100
