...
class KeywordResearcher:
    ...
    async def research_keyword(
        ...
        try:
            ...
            (
                ...
                product_analysis,
                conversion_rate,
            ) = await self._perform_keyword_analysis(keyword, category)
    ...
    def _get_cached_metrics(
        self, keyword: str, category: Optional[str]
    ...
    async def _save_and_cache_metrics(self, metrics: KeywordMetrics):
        ...
        self._keyword_cache[cache_key] = {"metrics": metrics, "timestamp": datetime.now()}
        await self._save_keyword_metrics(metrics)

    async def find_opportunity_keywords(
        self, category: str, min_volume: int = 1000, max_competition: str = "medium"
    ) -> List[Dict[str, Any]]:
        """
        기회 키워드 발굴
        ...
        try:
            ...
            opportunity_keywords = await self._filter_opportunity_keywords(
                all_keywords, category, min_volume, max_competition
            )
    ...
    async def _filter_opportunity_keywords(
        self, all_keywords: List[str], category: str, min_volume: int, max_competition: str
    ) -> List[Dict[str, Any]]:
        ...
        for keyword in all_keywords:
            ...
            if metrics.search_volume >= min_volume:
                comp_index = (
                    competition_levels.index(metrics.competition_level)
                    if metrics.competition_level in competition_levels
                    else 3
                )
    ...
    def _calculate_and_sort_opportunities(
        self, opportunity_keywords: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    async def analyze_keyword_combinations(
        self, base_keywords: List[str], category: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        키워드 조합 분석
        ...
        try:
            ...
            for combo_str, keywords_list in combinations_raw:
                tasks.append(
                    self._analyze_and_score_combination(combo_str, keywords_list, category)
                )
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if j + 1 < n:
                    ...
                    combinations.append(
                        (combo3, [base_keywords[i], base_keywords[j], base_keywords[j + 1]])
                    )
        return combinations

    async def _analyze_and_score_combination(
        self, combo_str: str, keywords_list: List[str], category: Optional[str]
    ...
    async def track_keyword_trends(self, keywords: List[str], days: int = 30) -> Dict[str, Any]:
        """
        키워드 트렌드 추적
        ...
        try:
            ...
            return {"keyword_trends": trends, "summary": summary}

        except Exception as e:
            logger.error(f"키워드 트렌드 추적 오류: {str(e)}")
            return {"keyword_trends": {}, "summary": {}}

    async def _get_keyword_daily_data(self, keyword: str, days: int) -> List[Dict[str, Any]]:
        """일별 키워드 데이터 조회"""
        ...
        daily_data = await self.storage.list(
            "keyword_daily_data",
            filters={"keyword": keyword, "date": {"$gte": start_date, "$lte": end_date}},
        )
    ...
    def _generate_trend_summary(self, trends: Dict[str, Any], days: int) -> Dict[str, Any]:
        """트렌드 요약을 생성합니다."""
        return {
            "rising_keywords": [k for k, v in trends.items() if v["trend"] == TrendDirection.UP],
            "declining_keywords": [
                k for k, v in trends.items() if v["trend"] == TrendDirection.DOWN
            ],
            "stable_keywords": [
                k for k, v in trends.items() if v["trend"] == TrendDirection.STABLE
            ],
    ...
    async def _get_search_volume(self, keyword: str) -> int:
        ...
        search_data = await self.storage.list(
            "keyword_search_data", filters={"keyword": keyword}, limit=1
        )
    ...
    async def _analyze_competition(self, keyword: str, category: Optional[str] = None) -> str:
        """경쟁도 분석"""
        # 해당 키워드 상품 수 조회
        filters = {}
        if category:
        ...
        products = await self.storage.list("products", filters=filters, limit=1000)
        ...
        search_volume = await self._get_search_volume(keyword)
        logger.debug(
            f"Keyword: {keyword}, Search Volume: {search_volume}, Matching Products: {matching_products}"
        )
        if search_volume == 0:
            return "low"
    ...
    async def _analyze_seasonality(self, keyword: str) -> Dict[str, float]:
        ...
        historical_data = await self.storage.list(
            "keyword_monthly_data", filters={"keyword": keyword}, limit=24  # 2년치
        )
        ...
        seasonality = {}
        total_avg = (
            sum(sum(volumes) / len(volumes) for volumes in monthly_data.values() if volumes) / 12
            if monthly_data
            else 100
        )
    ...
    async def _find_related_keywords(
        ...
        products = await self.storage.list("products", filters=filters, limit=100)
        ...
        for product in products:
            ...
            if keyword.lower() in name:
                ...
                for word in words:
                    if len(word) > 2 and word != keyword.lower() and word not in self.stopwords:
                        related.add(word)

        # 2. 검색 쿼리 로그에서 추출 (시뮬레이션)
        query_logs = await self.storage.list(
            "search_query_logs", filters={"query": {"$contains": keyword}}, limit=100
        )
    ...
    async def _analyze_products(
        ...
        products = await self.storage.list("products", filters=filters)
        ...
        return {"count": len(matching_products), "avg_price": avg_price}

    async def _estimate_conversion_rate(
        self, keyword: str, search_volume: int, product_count: int
    ) -> float:
        """전환율 추정"""
        # 실제로는 과거 판매 데이터 기반
        # 여기서는 간단한 추정
    ...
    def _estimate_revenue_potential(self, metrics: KeywordMetrics) -> Decimal:
        """수익 잠재력 추정"""
        # 월간 예상 판매량
        monthly_sales = (
    ...
    async def _collect_category_keywords(self, category: str) -> List[str]:
        """카테고리 관련 키워드 수집"""
        keywords = set()

        # 1. 상품명에서 추출
        products = await self.storage.list(
            "products", filters={"category_name": category}, limit=500
        )
        ...
        category_keywords = await self.storage.list(
            "category_keywords", filters={"category": category}
        )
    ...
    def _calculate_synergy_score(
        self, base_keywords: List[str], combination_metrics: KeywordMetrics
    ) -> float:
        """시너지 점수 계산"""
        # 개별 키워드 대비 조합의 효과
        ...
        # 전환율이 높으면 시너지 높음
        base_score = (
            combination_metrics.conversion_rate * 100 if combination_metrics.conversion_rate else 0
        )
    ...
    async def _get_daily_keyword_data(self, keyword: str, days: int) -> List[Dict[str, Any]]:
        """일별 키워드 데이터 조회"""
        end_date = datetime.now()
        start_date = end_date - timedelta(days=days)
    ...
    def _analyze_trend_data(self, daily_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """트렌드 데이터 분석"""
        if len(daily_data) < 2:
            return {"direction": TrendDirection.STABLE, "strength": 0.0, "volatility": 0.0}
    ...
    def _forecast_trend(self, daily_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """트렌드 예측"""
        if len(daily_data) < 7:
        ...
        # 간단한 예측 (이동평균 유지)
        return {"days": 7, "predicted_volume": int(ma7), "confidence": 70.0}  # 고정 신뢰도
