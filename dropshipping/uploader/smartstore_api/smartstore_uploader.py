...
class SmartstoreUploader(BaseUploader):
    ...
    async def transform_product(self, product: StandardProduct) -> Dict[str, Any]:
        ...
        smartstore_data = {
            "originProduct": {
                ...
                "images": {
                    "representativeImage": {
                        "url": str(product.images[0].url)
                    },
                    "optionalImages": [
                        {"url": str(img.url)}
                        for img in product.images[1:10]  # 최대 10개
                    ]
                },
    ...
    def _create_options(self, product: StandardProduct) -> Dict[str, Any]:
        ...
        option_info["options"].append({
            "groupName": variant.name if product.variants else "옵션",
    ...
    async def update_single(
        ...
        try:
            ...
            response = await self._api_request(
                "PUT",
                f"/v2/products/{marketplace_product_id}",
                json=product_data
            )
