...
class BaseUploader(ABC):
    ...
    def __init__(
        ...
        # 통계
        self.stats = {"uploaded": 0, "failed": 0, "skipped": 0, "errors": []}
    ...
    async def upload_product(
        self, product: StandardProduct, update_existing: bool = True
    ) -> Dict[str, Any]:
        """
        상품 업로드 (신규 또는 수정)
        ...
        try:
            ...
            if existing and update_existing:
                ...
                upload_result = await self.update_single(
                    existing["marketplace_product_id"], product_data
                )
            ...
            self.stats["errors"].append(
                {"product_id": product.id, "error": str(e), "timestamp": datetime.now()}
            )
    ...
    async def upload_batch(
        ...
        Returns:
            업로드 결과 목록
        """
        logger.info(f"{self.marketplace.value}에 {len(products)}개 상품 배치 업로드 시작")
        ...
        results = await asyncio.gather(
            *[upload_with_semaphore(product) for product in products], return_exceptions=True
        )
        ...
        logger.info(
            f"배치 업로드 완료: 성공 {success_count}, " f"실패 {len(valid_results) - success_count}"
        )
    ...
    async def _save_upload_record(self, product: StandardProduct, result: Dict[str, Any]):
        """업로드 기록 저장"""
        try:
            # 실제로는 storage에 저장
