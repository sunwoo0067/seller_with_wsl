...
async def get_product(
    product_id: str, storage: BaseStorage = Depends(get_storage), _: None = Depends(rate_limiter)
):
    """상품 상세 조회"""
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="상품을 조회할 수 없습니다"
        )
...
async def create_product(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="상품을 생성할 수 없습니다"
        )
...
async def update_product(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="상품을 수정할 수 없습니다"
        )
...
async def delete_product(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="상품을 삭제할 수 없습니다"
        )


@router.post("/{product_id}/sync")
async def sync_product(
    product_id: str, storage: BaseStorage = Depends(get_storage), _: None = Depends(require_api_key)
):
    """상품 동기화"""
    try:
        # 기존 상품 확인
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="상품을 동기화할 수 없습니다"
        )
...
async def get_product_history(
    ...
    try:
        ...
        # 전체 개수 조회
        total = await storage.count("product_history", filters={"product_id": product_id})
