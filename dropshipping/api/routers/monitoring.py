...
from dropshipping.api.dependencies import (
    get_storage, require_api_key, rate_limiter
)
from dropshipping.monitoring import (
    get_logger, global_metrics, alert_manager, 
    performance_tracker
)
...
async def send_test_alert(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="알림을 전송할 수 없습니다"
        )


@router.get("/performance")
async def get_performance_stats(
    operation: Optional[str] = Query(None, description="작업 이름"),
    _: None = Depends(rate_limiter)
):
    """성능 통계 조회"""
    try:
...
async def check_dependencies(
    ...
    try:
        ...
        overall_status = "healthy" if unhealthy_count == 0 else "degraded" if unhealthy_count < len(dependencies) else "unhealthy"
