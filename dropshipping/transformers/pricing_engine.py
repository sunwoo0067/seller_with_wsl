...
class PricingEngine:
    ...
    def calculate_price(
        ...
        """
        # 적용 가능한 규칙 찾기
        applicable_rules = self._find_applicable_rules(cost, supplier_id, category_code)
        ...
        # 가격 계산
        result = self._calculate_with_rule(cost, primary_rule, market_price, volume)
    ...
    def _find_applicable_rules(
        self, cost: Decimal, supplier_id: str = None, category_code: str = None
    ) -> List[PricingRule]:
        """적용 가능한 규칙 찾기"""
        applicable = []
    ...
    def _calculate_with_rule(
        self, cost: Decimal, rule: PricingRule, market_price: Decimal = None, volume: int = 1
    ) -> PriceCalculationResult:
        """특정 규칙으로 가격 계산"""
        calculation_details = {
            "rule_id": rule.id,
    ...
    def _apply_dynamic_adjustment(
        self, result: PriceCalculationResult, dynamic_rule: PricingRule
    ) -> PriceCalculationResult:
        """동적 조정 적용"""
