...
class AlertManager:
    ...
    def send_sync(
        ...
        """동기 알림 전송"""
        asyncio.create_task(self.send(title, message, level, source, metadata, error))
    ...
    async def _send_to_log(self, alert: Alert):
        ...
        log_func(
            f"[{alert.source}] {alert.title}: {alert.message}", extra={"alert": alert.to_dict()}
        )
    ...
    async def _send_to_slack(self, alert: Alert):
        ...
        if alert.error:
            payload["attachments"][0]["fields"].append(
                {"title": "Error", "value": f"```{str(alert.error)}```", "short": False}
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    settings.monitoring.slack_webhook_url, json=payload, timeout=10
                )
    ...
    async def _send_to_webhook(self, alert: Alert):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(webhook_url, json=alert.to_dict(), timeout=10)
                response.raise_for_status()
